...
$$T_{linear} \propto \frac{1}{g_{\min}^2} \propto N$$
...
$$P_{opt} \approx 1 - \epsilon^2$$

If we set the run time $T$ to be the optimal Grover time $T_{Grover} = \frac{\pi}{2} \sqrt{N}$:

$$T_{Grover} = \frac{\pi}{2} \sqrt{10^{15}} \approx 1.57 \times 31.6 \times 10^6 \approx 50 \times 10^6 \text{ steps}$$
...
$$P_{success}(T) = \sin^2\left( \frac{T}{T_{Grover}} \frac{\pi}{2} \right) \approx \left( \frac{T}{T_{Grover}} \frac{\pi}{2} \right)^2$$
...
$$P_{shot} \approx r^2 \times (\text{Geometric Factor}) \approx (10^{-3})^2 \times 2.5 \approx 2.5 \times 10^{-6}$$
