...
def build():
    ...
    for category, path, name in links:
        rel_path = f"../../{path}"
        # For the first table
        row1 = f'<tr class="border-b border-slate-700"><td class="px-4 py-2">{category}</td><td class="px-4 py-2"><a href="{rel_path}" class="text-violet-400 hover:text-white">View {name}</a></td></tr>'
        rows1.append(row1)
        # For the second table
        row2 = f'<tr class="border-b border-slate-800 hover:bg-slate-800/50 transition-colors"><td class="px-4 py-3 font-mono text-slate-300">{category}</td><td class="px-4 py-3"><a href="{rel_path}" class="text-violet-400 hover:text-violet-300 transition-colors">View {name}</a></td></tr>'
        rows2.append(row2)
    ...
    # We need to replace the content of tbody.
    content = re.sub(r'<tbody>.*?</tbody>', f'<tbody>\n{rows1_html}\n                </tbody>', content, count=1, flags=re.DOTALL)
    content = re.sub(r'<tbody id="archive-body">.*?</tbody>', f'<tbody id="archive-body">\n{rows2_html}\n                </tbody>', content, count=1, flags=re.DOTALL)
    ...
    """
    content = content.replace("</body>", f"{additional_ui}\n{json_script}\n</body>")
