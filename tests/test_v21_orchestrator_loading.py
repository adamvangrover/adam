...
class TestV21OrchestratorLoading(unittest.TestCase):
    ...
    def test_orchestrator_loads_all_v21_agents(self):
        ...
        try:
            ...
            expected_agent_names = [agent['name'] for agent in expected_agents if isinstance(agent, dict) and 'name' in agent]
            ...
            self.assertEqual(len(orchestrator.agents), len(expected_agent_names), "Number of loaded agents should match the number in the config file.")
