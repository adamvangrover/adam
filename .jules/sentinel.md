...
## 2026-01-26 - [Critical] Path Traversal in Code Deployment
...
## 2026-03-01 - Bypassing Security Standard via inline nosec
**Vulnerability:** The `TechnicalAnalystAgent` in `core/agents/technical_analyst_agent.py` was directly using `pickle.load()` despite a codebase-wide security standard mandating the use of `SafeUnpickler`. The vulnerability was masked from static analysis (Bandit) via an inline `# nosec B301` comment. This allowed arbitrary code execution (RCE) via insecure deserialization if a compromised model file was loaded.
**Learning:** Developers may use `# nosec` tags to bypass static analysis checks when implementing features quickly or reusing old code snippets, inadvertently undermining systemic security controls. A security standard is only effective if compliance is verified beyond simple static analysis exclusions.
**Prevention:** Replaced `pickle.load()` with `SafeUnpickler(f).load()` in the affected agent. To prevent recurrence, code reviews must strictly scrutinize the use of `# nosec` tags, particularly for critical vulnerabilities like deserialization or code execution, ensuring they are only used for legitimate, documented exceptions and not to bypass established security libraries.
