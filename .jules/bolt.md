...
## 2026-11-29 - [Zustand Store Subscriptions in High-Frequency Loops]
**Learning:** In `PromptAlpha.tsx`, calling a custom hook like `useSwarmSimulation()` that destructured the entire store state (e.g., `const { agents } = useSwarmStore()`) caused the parent component to subscribe to all state changes. Because the simulation loop updated `agents` 5 times a second, `PromptAlpha` underwent massive, continuous O(N) re-renders, causing severe UI lag.
**Action:** When creating hooks that run high-frequency simulations or side-effects, never destructure the full Zustand store if it causes parent components to subscribe unnecessarily. Extract only the invariant functions using selectors (e.g., `useSwarmStore(state => state.initializeSwarm)`), and retrieve the latest volatile state inside the loop using `useSwarmStore.getState()`. This cleanly decouples the state logic from the React render cycle.
