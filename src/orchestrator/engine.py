    ...
    async def _process_queue(self):
        async with aiosqlite.connect(self.db_path) as db:
            while not self._stop_event.is_set() or not self.queue.empty():
                try:
                    # Wait for an item, but don't block forever if stopping
                    action, data = await asyncio.wait_for(self.queue.get(), timeout=1.0)
                    ...
                    if action == "upsert_workflow":
                        ...
                        """, (data['workflow_id'], data['trace_id'], data['status'], data['start_time'], data['end_time'], json.dumps(data['global_context'])))
                        ...
                        """, (data['task_id'], data['workflow_id'], data['span_id'], data['parent_id'], data['status'], data.get('duration_sec'), data['updated_at']))
                    
                    elif action == "insert_log":
                        await db.execute("""
                            INSERT INTO execution_logs (trace_id, task_id, span_id, event_type, timestamp, metrics)
                            VALUES (?, ?, ?, ?, ?, ?)
                        """, (data['trace_id'], data['task_id'], data['span_id'], data['event'], data['timestamp'], json.dumps(data.get('metrics', {}))))
...
class StateLedger:
    def __init__(self, workflow_id: str, db_manager: AsyncSQLiteManager, trace_id: Optional[str] = None):
    ...
    def _sync_workflow_state(self):
        ...
        self.db.queue.put_nowait(("upsert_workflow", {
    ...
    def log_event(self, event: str, task_id: str, span_id: str, metrics: Optional[Dict[str, Any]] = None):
        ...
        if event == "TASK_FAILED":
            logger.error(f"{log_msg} | Reason: {entry['metrics'].get('error', 'Unknown')}")
        else:
            logger.debug(log_msg)

    def update_task_status(self, task_id: str, status: str, span_id: str, parent_id: Optional[str] = None, duration: Optional[float] = None):
        ...
        # Enqueue for DB persistence
        db_payload = {"task_id": task_id, "workflow_id": self.workflow_id, **update_data}
        self.db.queue.put_nowait(("upsert_task", db_payload))
        
        # Sync workflow state if context might have updated
...
class OrchestratorEngine:
    ...
    def add_task(self, task: TaskNode):
        ...
        if task.task_id not in self.ledger.task_registry:
            self.ledger.update_task_status(task.task_id, TaskState.PENDING, f"sp-{uuid.uuid4().hex[:8]}", None)
    ...
    def validate_dag(self):
        ...
        def dfs(node_id: str):
            if node_id in path:
                raise CircularDependencyError(f"Circular dependency detected involving task: {node_id}")
            if node_id in visited:
                return
            
            path.add(node_id)
            for dep in self.tasks.get(node_id, TaskNode(node_id, lambda: None)).dependencies:
    ...
    def _get_parent_span_id(self, task: TaskNode) -> Optional[str]:
        if not task.dependencies:
            return None
        return self.ledger.task_registry.get(task.dependencies[0], {}).get("span_id")

    async def _execute_with_retry(self, task: TaskNode, span_id: str, parent_id: Optional[str]) -> str:
        attempt = 0
        ...
        while attempt <= task.max_retries:
            try:
                result = await asyncio.wait_for(
                    task.coroutine_func(self.ledger.global_context),
                    timeout=task.timeout
                )

                if task.is_human_gate and result == "SUSPENDED_AWAITING_INPUT":
                    self.ledger.update_task_status(task.task_id, TaskState.SUSPENDED_AWAITING_INPUT, span_id, parent_id)
                    self.ledger.log_event("TASK_SUSPENDED", task.task_id, span_id, metrics={"reason": "Awaiting human input"})
                ...
                self.ledger.update_task_status(task.task_id, TaskState.COMPLETED, span_id, parent_id, duration=duration)
                self.ledger.log_event("TASK_SUCCESS", task.task_id, span_id, metrics={"tokens_used": tokens_used, "duration_sec": round(duration, 3)})
            ...
            if attempt <= task.max_retries:
                ...
                self.ledger.log_event("TASK_RETRY", task.task_id, span_id, metrics={"attempt": attempt, "delay": delay, "error": err_msg})
                await asyncio.sleep(delay)
            else:
                duration = time.perf_counter() - start_time
                self.ledger.update_task_status(task.task_id, TaskState.FAILED, span_id, parent_id, duration=duration)
                self.ledger.log_event("TASK_FAILED", task.task_id, span_id, metrics={"error": err_msg})
                return TaskState.FAILED

    async def _run_task_wrapper(self, task: TaskNode, semaphore: asyncio.Semaphore):
        async with semaphore:
            ...
            if task.conditional_router and not task.conditional_router(self.ledger.global_context):
                self.ledger.update_task_status(task.task_id, TaskState.SKIPPED, span_id, parent_id)
                self.ledger.log_event("TASK_SKIPPED", task.task_id, span_id, metrics={"reason": "Conditional Router evaluated to False"})
                return task.task_id, TaskState.SKIPPED

            self.ledger.update_task_status(task.task_id, TaskState.RUNNING, span_id, parent_id)
            self.ledger.log_event("TASK_START", task.task_id, span_id)

            final_status = await self._execute_with_retry(task, span_id, parent_id)
    ...
    async def run(self, max_concurrency: int = 10):
        ...
        if self.ledger.status not in [TaskState.SUSPENDED_AWAITING_INPUT, TaskState.FAILED]:
            self.ledger.status = TaskState.RUNNING
            self.ledger._sync_workflow_state()

        semaphore = asyncio.Semaphore(max_concurrency)
        running_coros = set()
        pending_tasks = {t_id for t_id, state in self.ledger.task_registry.items() if state.get("status") == TaskState.PENDING}
        ...
        while self.ledger.status == TaskState.RUNNING:
            ...
            for task_id in list(pending_tasks):
                ...
                deps_statuses = [self.ledger.task_registry.get(dep, {}).get("status") for dep in task.dependencies]
                
                if any(s in (TaskState.FAILED, TaskState.SKIPPED) for s in deps_statuses):
                    span_id = self.ledger.task_registry.get(task_id, {}).get("span_id") or f"sp-{uuid.uuid4().hex[:8]}"
                    self.ledger.update_task_status(task_id, TaskState.SKIPPED, span_id, self._get_parent_span_id(task))
                    self.ledger.log_event("TASK_SKIPPED", task_id, span_id, metrics={"reason": "Upstream dependency failed or skipped"})
            ...
            for task in ready_to_launch:
                task_future = asyncio.create_task(self._run_task_wrapper(task, semaphore))
                running_coros.add(task_future)

            if not running_coros:
                break

            done, running_coros = await asyncio.wait(running_coros, return_when=asyncio.FIRST_COMPLETED)
        ...
        all_completed = all(self.ledger.task_registry.get(t, {}).get("status") in (TaskState.COMPLETED, TaskState.SKIPPED) for t in self.tasks)
        
        if self.ledger.status != TaskState.SUSPENDED_AWAITING_INPUT:
            if all_completed:
                self.ledger.status = TaskState.COMPLETED
            elif any(self.ledger.task_registry.get(t, {}).get("status") == TaskState.FAILED for t in self.tasks):
                self.ledger.status = TaskState.FAILED

            self.ledger.end_time = self.ledger._get_timestamp()
            self.ledger._sync_workflow_state()
        
        logger.info(f"Orchestration run ended with status: {self.ledger.status}")
        
        # Ensure the DB queue is drained and safely closed
        await self.db_manager.stop()

    def resolve_human_gate(self, task_id: str, callback_payload: Dict[str, Any]):
        if self.ledger.task_registry.get(task_id, {}).get("status") == TaskState.SUSPENDED_AWAITING_INPUT:
            ...
            self.ledger.update_task_status(task_id, TaskState.COMPLETED, span_id, parent_id)
