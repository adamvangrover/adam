...
class RiskEntity(BaseModel):
    ...
    quantitative_exposure: float = Field(...,
                                         description="Financial Value at Risk (VaR) or Single Loss Expectancy (SLE).")
    control_effectiveness: float = Field(..., description="0.0 to 1.0 score of mitigation reliability.")
    control_strength: str = Field(...,
                                  description="Qualitative assessment of control reliability: Strong, Moderate, Weak.")
...
class CrisisSimulationOutput(BaseModel):
    ...
    crisis_simulation_log: List[CrisisLogEntry] = Field(...,
                                                        description="A chronological timeline of simulated events.")
