...
class Sec13FHandler:
    ...
    def __init__(self):
        self.headers = {"User-Agent": "Adam/2.0 (internal-research@adam-project.io)"}
        ...
        self.mock_data = {
            "0001067983": {  # Berkshire Hathaway
                ...
                "2025-Q2": [{"ticker": "AAPL", "value": 140000000000, "shares": 600000000, "action": "HOLD"}],
            },
            "0001037389": {  # Renaissance Technologies
                ...
                "2025-Q2": [{"ticker": "PLTR", "value": 120000000, "shares": 4900000, "action": "HOLD"}],
            },
        }
    ...
    def calculate_delta(self, current_df: pd.DataFrame, previous_df: pd.DataFrame) -> pd.DataFrame:
        ...
        merged = pd.merge(current_df, previous_df, on=join_key, how="outer", suffixes=("_curr", "_prev"))
        ...
        merged["pct_change"] = (
            (merged["share_change"] / merged["shares_prev"]).replace([float("inf"), -float("inf")], 0).fillna(0)
        )

        # ⚡ Bolt: Vectorize action calculation using np.select instead of apply for ~200x speedup
        conditions = [
