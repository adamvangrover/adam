...
class DataRetrievalAgent(AgentBase):
    ...
    def __init__(self, config: Dict[str, Any], **kwargs):
        ...
        self.description = self.config.get(
            'description', "Retrieves data from various configured sources based on structured requests.")
    ...
    def _fetch_real_company_data(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            total_liabilities = get_mapped_series(
                balance_transposed, ["Total Liabilities Net Minority Interest", "Total Liabilities"])
            equity = get_mapped_series(
                balance_transposed, ["Stockholders Equity", "Total Equity Gross Minority Interest"])
            ...
            if market_data.get("market_cap") and market_data.get("current_price"):
                financial_data_detailed["market_data"]["shares_outstanding"] = market_data["market_cap"] / \
    ...
    def _get_mock_abc_test_data(self):
        return {
            ...
            "collateral_and_debt_details": {
                ...
                "debt_tranches": [
                    {"type": "Senior Secured Term Loan", "amount": 300,
                     "interest_rate": "SOFR+300", "maturity": "2028-12-31"},
                    {"type": "Revolving Credit Facility", "amount": 200,
                     "interest_rate": "SOFR+250", "maturity": "2027-12-31"}
                ],
