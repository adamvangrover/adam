...
class FundamentalAnalystAgent(AgentBase):
    ...
    def calculate_financial_ratios(self, company_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        try:
            ...
            ratios["current_ratio"] = total_assets / \
                total_liabilities if total_liabilities else None  # Simplified current ratio
    ...
    def assess_financial_health(self, financial_ratios: Dict[str, float]) -> str:
        ...
        if financial_ratios.get("net_profit_margin", 0) > .1:
            score += 1
        if financial_ratios.get("revenue_growth") is not None and financial_ratios.get("revenue_growth", 0) > 0.05:
                ...
                if financial_ratios:
                    for ratio, value in financial_ratios.items():
                        ratios_str_parts.append(
                            f"  - {ratio}: {value:.2f}" if value is not None else f"  - {ratio}: N/A")
    ...
    def calculate_dcf_valuation(self, company_data: Dict[str, Any]) -> Optional[float]:
        ...
        try:
            ...
            essential_rates = [discount_rate, terminal_growth_rate_perpetuity,
                               initial_high_growth_rate, stable_growth_rate]
            ...
            terminal_value = (terminal_value_fcf_base * (1 + terminal_growth_rate_perpetuity)) / \
    ...
    # Should be financial_ratios based on usage
