                ...
                if period == "daily":
                    ...
                    if response.status_code == 200:
                        ...
                        price_data = [{'date': item['date'], 'open': item['open'], 'high': item['high'],
                                       'low': item['low'], 'close': item['close'], 'volume': item['volume']} for item in data]
                    else:
            ...
            if alpha_vantage_key is None:
                ...
                if period == "daily":
                    ...
                    if response.status_code == 200:
                        ...
                        if data:
                            price_data = [{'date': date, 'open': float(item['1. open']), 'high': float(item['2. high']), 'low': float(
                                item['3. low']), 'close': float(item['4. close']), 'volume': int(item['5. volume'])} for date, item in data.items()]
                        else:
                            logging.error(
                                f"Error fetching data from Alpha Vantage: 'Time Series (Daily)' not found in response. Response: {response.text}")
                            # price_data remains None
                    else:
                        logging.error(
                            f"Error fetching data from Alpha Vantage: {response.status_code} - {response.text}")
                        # price_data remains None
                elif period == "intraday":
                    ...
                    if response.status_code == 200:
                        ...
                        if data:
                            ...
                            price_data = [{'date': date_time.split()[0], 'minute': date_time.split()[1][:5] if len(date_time.split()) > 1 else None, 'open': float(item['1. open']), 'high': float(
                                item['2. high']), 'low': float(item['3. low']), 'close': float(item['4. close']), 'volume': int(item['5. volume'])} for date_time, item in data.items()]
                        else:
                            logging.error(
                                f"Error fetching data from Alpha Vantage: 'Time Series (5min)' not found in response. Response: {response.text}")
                            # price_data remains None
                    else:
                        logging.error(
                            f"Error fetching data from Alpha Vantage: {response.status_code} - {response.text}")
