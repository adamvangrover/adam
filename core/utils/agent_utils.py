...
def share_knowledge_between_agents(sender_agent: str, receiver_agent: str, knowledge_type: str, knowledge_data: Any) -> None:
    """
    ...
    try:
        ...
        logger.debug(f"Knowledge ({knowledge_type}) shared from {sender_agent} to {receiver_agent}")
    except Exception as e:
        logger.error(f"Error in knowledge sharing between agents: {e}")
...
def monitor_agent_performance(agent_name: str, metric: str, value: Any) -> None:
    ...
    """
    # Simply log the metric for now. Real implementation could push to Datadog/Prometheus.
    logger.info(f"PERFORMANCE | Agent: {agent_name} | Metric: {metric} | Value: {value}")
...
def validate_agent_inputs(agent_name: str, inputs: Dict[str, Any], required_parameters: List[str]) -> None:
    ...
    if missing:
        ...
        logger.error(f"Agent {agent_name} missing required parameters: {missing_str}")
        raise ValueError(f"Agent {agent_name} missing required parameter: {missing[0]}")

def format_agent_output(agent_name: str, output_data: Any, format: str = "json") -> str:
    """
    ...
    match format.lower():
        case "json":
            try:
                ...
                logger.error(f"JSON serialization failed for agent {agent_name}: {e}")
                return json.dumps({"error": "Unserializable data"})
        case "csv":
            if not isinstance(output_data, list) or not all(isinstance(i, dict) for i in output_data):
                raise ValueError(f"Agent {agent_name} output must be a list of dicts for CSV format.")
            if not output_data:
                return ""
            output = io.StringIO()
...
def log_agent_action(agent_name: str, action: str, details: Any) -> None:
    ...
    """
    logger.info(f"ACTION | Agent: {agent_name} | Action: {action} | Details: {details}")

def parse_json_garbage(text: str) -> Dict[str, Any]:
    """
    Safely extracts and parses JSON from a string that might contain markdown or conversational garbage.
    ...
    # 2. Try to extract JSON blocks from markdown
