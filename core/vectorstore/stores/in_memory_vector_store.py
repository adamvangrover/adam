...
class InMemoryVectorStore(BaseVectorStore):
    ...
    async def add_documents(self, documents: List[Tuple[str, List[float]]]):
        ...
        for text, embedding_list in documents:
            if len(embedding_list) != self.embedding_dim:
                logging.error(
                    f"InMemoryVectorStore: Embedding dimension mismatch. Expected {self.embedding_dim}, got {len(embedding_list)} for text: '{text[:50]}...'")
    ...
    async def search(self, query_embedding: List[float], top_k: int = 5) -> List[Tuple[str, float]]:
        ...
        if len(query_embedding) != self.embedding_dim:
            logging.error(
                f"InMemoryVectorStore: Query embedding dimension mismatch. Expected {self.embedding_dim}, got {len(query_embedding)}.")
            return []
