...
class PersistentIngestionStrategy:
    ...
    def __init__(self, storage_path: str = "data/vector_store", chunking_strategy: str = "recursive"):
        ...
        self.model = None
        if chunking_strategy == "semantic" and SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                self.model = SentenceTransformer('all-MiniLM-L6-v2')
            except Exception as e:
                logger.error(f"Failed to initialize sentence_transformers in PersistentIngestionStrategy: {e}")
    ...
    async def ingest(self, data_source: Any, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with aiofiles.open(output_file, 'w', encoding='utf-8') as f:
            for chunk in chunks:
                ...
                if self.model and "text" in chunk:
                    try:
                        # Ensure we do non-blocking inference for embeddings
                        embedding = await asyncio.to_thread(self.model.encode, chunk["text"])
                        chunk["embedding"] = embedding.tolist()
                    except Exception as e:
                        logger.error(f"Failed to encode chunk embedding: {e}")
